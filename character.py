...
def character_info(player_name, player_stats, temp_stats):
    ...
    print("-" * 60)
    print(f"{skills['name'].title():<16}{skills['health']:<6}{skills['strength']:<6}{skills['dexterity']:<6}{skills['intelligence']:<6}")
...
def profession_list():
    ...
    for prof_select, skill in SELECT_PROFESSION.items():
        print(f"{prof_select}) {skill['name'].title():<12}{skill['health']:<6}{skill['strength']:<6}{skill['dexterity']:<6}{skill['intelligence']:<6}")
    print("-" * 60)
